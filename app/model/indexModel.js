...
class IndexModel {
	...
	GetCategoriasAtualizacoesFaculdade(POST) {
		return new Promise(function(resolve, reject) {
			...
			var values = [POST.icone,POST.id_categoria,POST.id_categoria,0,POST.id_faculdade,POST.id_categoria,0,POST.id_usuario,0,0,0,POST.id_usuario,POST.id_categoria,POST.id_usuario];
			
			helper.Query('SELECT COUNT(id) as atualizacoes, ? as icone,\
				(SELECT id FROM postagens_categorias WHERE id = ? ) as id,\
				(SELECT nome FROM postagens_categorias WHERE id = ? ) as nome\
