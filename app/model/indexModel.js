...
class IndexModel {
	...
	GetPostagensTodas(POST) {
		return new Promise(function(resolve, reject) {
			...
			if (POST.id_usuario != 1) {
				...
				OR (\
				(id_grupo = ? AND deletado = ? OR id_grupo IN (SELECT id_grupo FROM grupos_usuarios WHERE id_usuario = ? AND deletado = ?))\
				AND (id_contato = ? AND deletado = ? OR id_contato IN (SELECT id_usuario2 FROM usuarios_contatos WHERE id_usuario2 = ? AND deletado = ?))\
				)\
				AND (\
				(id_tipo = ? AND deletado = ? AND id_faculdade = ? AND (filtro_status_faculdade = ? OR filtro_status_faculdade = ?)) OR (id_tipo != ?)\
				)";
				values = [0, POST.id_usuario, 0, 0, 0, 0, POST.id_usuario, 0, 0, POST.id_usuario, 0, 0, 0, POST.id_usuario, 0, 2, 0, POST.id_faculdade,0, POST.status, 2];
			} else {
	...
	GetPostagensTodasFaculdade(POST){
		return new Promise(function(resolve, reject) {
			...
			/*Verifico se o aluno está na faculdade,ou se ele pertence ao grupo da postagem, 
			ou se ele pertence ao contato da postagem*/
